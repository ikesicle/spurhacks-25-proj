import React, { useState, useEffect } from 'react';
import ScriptView from './scriptview';

const Library = ({ setWindow, setCurrentScript, allScripts, setAllScripts }) => {
    const [counter, setCounter] = useState(0);

    useEffect(() => {
        (async () => {
            var response = await fetch('http://localhost:8000/scripts/get_scripts', {
                method: 'GET'});
            if (response.ok) {
                var data = await response.json();
                console.log(data);
                setAllScripts(data);
            }

        })()}, [counter]);

    return (
        <div className="flex flex-col h-screen bg-gray-100">
            <div className="flex flex-1 w-auto justify-center items-start m-5">
                <div className="flex flex-col items-center flex-1 transition-all duration-300 relative">
                    <div className="w-3/5 max-w-xl relative">
                        <div className="w-full flex flex-row items-center gap-2">
                            <h2 className="text-lg font-semibold flex-1">Library</h2>
                            <div className="h-full">
                                <input
                                    type='file'
                                    id="file-select"
                                    name='Add Script From Disk'
                                    className="hidden"
                                    onChange={(e) => {
                                        setCurrentScript({
                                            name: e.target.files[0].name,
                                            path: window.electronAPI.getFilePath(e.target.files[0]),
                                        })
                                        setWindow('edit')
                                    }} />
                                <label htmlFor='file-select' className="h-full px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition z-30">
                                    Add Script From Disk
                                </label>
                            </div>
                            <button
                                className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition z-30"
                                onClick={() => setWindow('chat')}>
                                Back to Chat
                        </button>
                        </div>
                        
                        {allScripts.length === 0 ? (
                            <div className="text-gray-400">No scripts available.</div>
                        ) : (
                            <ul className="list-disc">
                                {allScripts.map((script, idx) => (
                                    <ScriptView scriptData={script} activateEdit={()=>{
                                        setCurrentScript(script);
                                        setWindow('edit');
                                    }} updateScript={()=>{setCounter(counter + 1)}} />
                                ))}
                            </ul>
                        )}
                    </div>
                </div>
            </div>
        </div>
    );
}

export default Library;